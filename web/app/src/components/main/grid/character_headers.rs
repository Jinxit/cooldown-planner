...
use crate::serverfns::{realms_for_character, GuildRosterEntry};
...
use fight_domain::{CharacterUuid, Lookup};
...
where
    G: Fn(UiCharacterTemplate) + Copy + 'static,
    H: Fn() + Copy + 'static,
{
    let ui_state = expect_context::<UiState>();
    let uuid = ui_character.uuid;
    ...
    let class_specs = Signal::<Vec<(LocalizedString, Option<LocalizedString>)>>::derive({
        let ui_character = ui_character.clone();
        move || {
            let class_spec_index = expect_context::<ClassSpecIndex>();
            if let Some(class) = &ui_character.class {
                class_spec_index
        ...
        >
            <div class="-ml-6 flex">
                ...
                >
                    ...
                    <Show when=move || !editing() fallback=|| () clone:ui_character>
                    ...
                    <Show clone:replace_ui_character clone:on_blur when=editing fallback=|| ()>
                ...
                <Show when=picking fallback=|| ()>
                    <ul class="absolute z-30 mt-6 rounded-md border-2 border-slate-950 bg-slate-800 text-left text-sm font-normal text-slate-300 cursor-pointer">
                        <For
                            each=move || class_specs().into_iter().enumerate().map(|(index, class_spec)| (index == 0, class_spec))
                            key=move |(_, class_spec)| class_spec.clone()
                            view=move | (is_first, (class, spec))| {
                                let class2 = class.clone();
                                view! {
                                        <li class="flex w-full hover:bg-slate-700"
                                            on:click={
                                        ...
                                        >
                                            <div
                                                class=move || format!("border-r-cc-{}", class.get(Locale::EnglishUnitedStates).to_case(Case::Kebab)) + " w-5 box-content border-r-2"
                                            ></div>
                                            <div
                                            ...
                                            >
                                                {move || spec.as_ref().map(|s| format!("{} ", s.localize()))}{move || class2.localize()}
                                            </div>
                                        </li>
                                }
                            }
    ...
    G: Fn() + Copy + 'static,
{
    ...
    let character_realm = Signal::derive({
        ...
        move || {
            let user_realm = expect_context::<PlannerContext>()
                .user
                .get()
                .map(|u| u.realm.name);
    ...
    let character_spec = store_value(format!(
        "\u{200B}{}{}",
        ui_character
            .spec
        ...
        <div class=move || {
            ...
            >
                ...
                >
                    ...
                    {move || character_realm().map(|realm| {
                        view! {
                            <span
    ...
    H: Fn() + Copy + 'static,
{
    let ui_state = expect_context::<UiState>();
    let autocomplete = {
        let guild_roster = expect_context::<GuildRoster>();
        let class_cache = expect_context::<ClassCache>();
        let planner_context = expect_context::<PlannerContext>();
        move |query: Signal<String>| {
            let realms_for_character = create_resource(
            ...
            let hits = create_memo(move |_| {
                ...
                if let Some((query_name, query_realm)) = query.split_once('-') {
                    ...
                    fuzzy_search(&query, guild_roster.into_iter()).collect::<Vec<RosterEntry>>()
                }
            });
            (move || {
                hits()
    ...
    let (selected_entry, set_selected_entry) = create_signal::<Option<RosterEntry>>(None);
                ...
                if let Some(first_spec) = first_spec {
                    let ui_character = UiCharacterTemplate::new_known(
                        ...
                        entry
                            .realm
                            .or_else(move || {
                                let user = expect_context::<PlannerContext>().user;
                                user.get().map(|u| u.realm)
                            })
                            .unwrap(),
                        class,
                        first_spec,
    ...
    view! {
        <Suspense fallback=|| ()>
            <AutocompleteDropdown
                ...
                key=move |entry| {
                    format!(
                        "{}-{}", &entry.name, entry.realm.as_ref().map(| r | r.slug.clone())
                        .unwrap_or_default()
                    )
                }
                view=move | entry, query, is_selected| {
                    let planner_context = expect_context::<PlannerContext>();
                    let region = create_memo(move |_| planner_context.region.get());
                    ...
                    let class_border = Signal::derive( {
                        ...
                        move || {
                            class
                                ...
                                .map(|c| {
                                    format!(
                                        "border-cc-{}", c.get(Locale::EnglishUnitedStates)
                                        .to_case(Case::Kebab)
                                    )
                                })
                    ...
                    let name = Signal::derive( {
                        ...
                        move || {
                            let realm = if query.contains('-') {
                                entry.realm.as_ref().map(|r| format!("-{}", r.name.localize())).unwrap_or_default()
                            } else {
                                "".to_string()
                            };
                    ...
                    view! {
                        ...
                        >
                            <BlankSuspense>
                                <div class=move || format!("{} my-1 border-l-2 mr-1 text-xs", class_border())>
                                    "\u{200b}"
                                </div>

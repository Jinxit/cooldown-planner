...
use fight_domain::{Attack, AttackUuid, CharacterUuid, Lookup, Spell};
use leptos::*;
        ...
        >
            {
                ...
                let attack_timer = move || ui_state.attacks().get(&attack_uuid).as_ref().unwrap().timer.static_timer();
                view! {
                    <div
                        ...
                        >
                            {
                                let is_assignable = Signal::derive({
                                    ...
                                    move || ui_state.is_spell_assignable(&assignment)
                                });

                                move || {
                                    let ui_character = ui_state.ui_characters().get(&character_uuid).cloned().unwrap();
                                    let spell = ui_character.spells.get(&assignment.spell).cloned().unwrap();
...
fn AssignmentIcon(
    ...
    let is_assigned = move || assignment.state != UiAssignmentState::Unassigned;
    let is_not_assigned = move || !is_assigned();
    let is_not_forced = move || assignment.state != UiAssignmentState::Forced;
    let is_forced = move || assignment.state == UiAssignmentState::Forced;

...
pub async fn realms_for_character(
    ...
    use crate::serverfns::util::{get_bnet_client, get_session, storage.try_fetch};
    ...
    async fn inner(
        ...
        let client = get_bnet_client(region).await?;
        let realms = storage.try_fetch(&(region,), Duration::from_secs(24 * 60 * 60), move || {
            realms(region)
        })
        .await?;

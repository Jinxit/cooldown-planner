    ...
    G: Fn() + Copy + 'static,
{
    ...
    let character_realm = Signal::derive({
        ...
        move || {
            let user = use_context::<PlannerContext>().unwrap().user.clone();
            let user_realm = user.get().map(|u| u.realm.name);
    ...
    H: Fn() + Copy + 'static,
{
    ...
    let autocomplete = {
        ...
        move |query: Signal<String>| {
            let realms_for_character = Resource::new(
                move || (region(), query()),
        ...
        <Suspense fallback=|| ()>
            <AutocompleteDropdown
                ...
                view=move |entry, query, is_selected| {
                    let planner_context = use_context::<PlannerContext>().unwrap();
                    let region = planner_context.region.clone();
                    let region = Memo::new(move |_| region());
